...
@implementation GSChatSync
+ (instancetype)init {
    return [[GSChatSync alloc] init];
}
- (instancetype)init{
    if (self = [super init]){
...
- (void) marshal:(id<GSWriter>) writer {
	...
	[writer WriteUInt32 :_Offset];


    [writer WriteByte :(UInt8)GSTagI32];
	[writer WriteUInt32 :_Count];
...
- (GSResponse*) Dispatch:(GSRequest*)call {
    switch(call.Method){
        ...
        case 2:{
            ...
            GSChatSync* ret = [ _service Sync: arg0  withArg1:arg1 ];
        ...
        case 4:{
...

			{
				...
				for(UInt16 i4 = 0; i4 < imax4; i4 ++ ){
					...
					[ arg0 addObject: [[NSNumber alloc] initWithUnsignedInt:v4]];

				}
			}


            NSMutableArray * ret = [ _service Fetch: arg0 ];
    ...
    return GSCreatePromise(_channel,call,^id<GSPromise>(GSResponse* response,id block,NSError **error){
        ...
        if(response.Exception != (SInt8)-1) {
            switch(response.Exception){
                ...
                case 0:{
					GSChatUserNotFound* callreturn = [[GSChatUserNotFound alloc] init];
                ...
                case 1:{
					GSChatUnexpectSQID* callreturn = [[GSChatUnexpectSQID alloc] init];
    ...
    return GSCreatePromise(_channel,call,^id<GSPromise>(GSResponse* response,id block,NSError **error){
        ...
        if(response.Exception != (SInt8)-1) {
            switch(response.Exception){
                ...
                case 1:{
					GSChatResourceNotFound* callreturn = [[GSChatResourceNotFound alloc] init];
                ...
                case 2:{
					GSChatResourceBusy* callreturn = [[GSChatResourceBusy alloc] init];
		...
		GSChatSync* callreturn = [[GSChatSync alloc] init];
        ...
        return ((id<GSPromise>(^)(GSChatSync*))block)(callreturn);;
    });
...
- (id<GSPromise>) Fetch:(NSMutableArray *) arg0 {
    ...
    NSMutableArray * params = [NSMutableArray array];
	{
		...
		for(id v2 in arg0){
			UInt32 vv2 = (UInt32)((NSNumber*)v2).unsignedIntValue;
			[writer WriteUInt32 :vv2];
		}
		GSParam *param  = [GSParam init];
    ...
    return GSCreatePromise(_channel,call,^id<GSPromise>(GSResponse* response,id block,NSError **error){
...

		{
			...
			for(UInt16 i3 = 0; i3 < imax3; i3 ++ ){
				...
				GSChatMail* v3 = [[GSChatMail alloc] init];
    ...
    return GSCreatePromise(_channel,call,^id<GSPromise>(GSResponse* response,id block,NSError **error){
        ...
        if(response.Exception != (SInt8)-1) {
            switch(response.Exception){
                ...
                case 1:{
					GSChatUserAuthFailed* callreturn = [[GSChatUserAuthFailed alloc] init];
